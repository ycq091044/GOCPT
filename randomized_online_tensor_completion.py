...
if __name__ == '__main__':
    ...
    # the mask streaming
    base, interval, preIter = 0.1, 0.009, 10
    mask_list = []
    ...
    for i in range(100):
        ...
        toc = time.time()
        rec, loss, PoF = metric(A_, B_, C_, X); result_CPD.append(PoF)
    ...
    for i in range(preIter):
        ...
        toc = time.time()
        rec, loss, PoF = metric(A, B, C, X, mask_base); result_pre.append(PoF)
        print ('loss:{}, PoF:{}, time:{}'.format(loss, PoF, toc-tic))
        tic = time.time()
    ...
    for mask_item in mask_list:
        ...
        toc = time.time()
        rec, loss, PoF = metric(A_, B_, C_, X, mask_base_); result_stream.append(PoF)
        print ('loss:{}, PoF:{}, time:{}'.format(loss, PoF, toc-tic))
        tic = time.time()
    ...
    for index, mask_item in enumerate(mask_list):
        mask_base_ += mask_item
        for i in range(1):
            A_, B_, C_ = iterationCPC(A_, B_, C_, A, B, C, mask_item, X, alpha= interval / (base + index*interval))
            A, B, C = A_.copy(), B_.copy(), C_.copy()
        toc = time.time()
        rec, loss, PoF = metric(A, B, C, X, mask_base_); result_cpc.append(PoF)
        print ('loss:{}, PoF:{}, time:{}'.format(loss, PoF, toc - tic))
        tic = time.time()
    ...
    plt.figure(1)
    plt.plot(result_CPD, label="Oracle CPD")
    plt.plot(result_stream, label="EM CPD (on base tensor) + EM CPD (add)")
    plt.plot(result_cpc, label="EM CPD (on base tensor) + row-wise LS")
    plt.legend()
    plt.ylabel('PoF')
